...
public class TopologyInstance {
	...
	protected ArrayList<Route> yens(DatapathId src, DatapathId dst, Integer K) {
		...
		for (int k = 1; k < K; k++) {
			...
			for (int i = 0; i <= A.get(k - 1).getPath().size() - 2; i = i + 2) {
				...
				for (Route r_A : A){
					for (Route r_B : B) {
						if (!r_A.getPath().equals(totalPath.getPath()) && !r_B.getPath().equals(totalPath.getPath())) {
							B.add(totalPath);
						}
					}
				}
	...
	protected Route getRoute(ServiceChain sc, DatapathId srcId, OFPort srcPort,
		...
		/* Could not find path b/t different switches */
		...
		/* Manually create path if b/t ports of same switch */
		if (r == null && srcId.equals(dstId)) {
			...
			return r;
		} else { /* Otherwise we don't know the path b/t different switches */
			return null;
		}
	}
